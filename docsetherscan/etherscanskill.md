...
- **eth_call**: Execute contract calls without creating transactions using `module=proxy&action=eth_call`
- **eth_getBlockByNumber**: Retrieve block data using `action=ethgetblockbynumber`
- **eth_getTransactionByHash**: Get transaction details using `action=ethgettransactionbyhash`
- **eth_getTransactionReceipt**: Retrieve transaction receipt using `action=ethgettransactionreceipt`
- **eth_getCode**: Get contract bytecode using `action=ethgetcode`
- **eth_getStorageAt**: Query contract storage using `action=ethgetstorageat`
- **eth_sendRawTransaction**: Broadcast signed transactions using `action=ethsendrawtransaction`
- **eth_estimateGas**: Estimate gas for transactions using `action=ethestimategas`
